...
def main() -> None:
    ...
    names = args.names
    if not names:
        names = sorted({path.stem for path in args.real_dir.glob("*.pt")} &
                       {path.stem for path in args.expect_dir.glob("*.pt")})
    print(f"{'name':<10} {'abs_mean':>12} {'abs_max':>12} {'rel_mean':>12} {'rel_p99':>12} {'cosine':>12} finite")
    for name in names:
        ...
        if not real_path.exists() or not expect_path.exists():
            ...
            continue
        result = stats(
            torch.load(real_path, map_location="cpu", weights_only=True),
            torch.load(expect_path, map_location="cpu", weights_only=True),
        )
        print(
            f"{name:<10} {result['abs_mean']:12.4e} {result['abs_max']:12.4e} "
