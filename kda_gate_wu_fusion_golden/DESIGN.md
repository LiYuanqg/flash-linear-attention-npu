...
| --- | --- | --- | ---: | ---: | --- |
| `in_g` | `[BT,BK]` | BF16 | 2 | 16 KiB | MTE2 ping-pong |
| `work` | `[BT,BK]` | FP32 | 1 | 16 KiB | 先 `g_corr`，再原地 `gk` |
| `tmp` | `[BT,BK]` | FP32 | 1 | 16 KiB | sigmoid / `exp2` |
| `dt_bias` | `[BK]` | FP32 | 1 | 256 B | 本 K tile 常驻 |
| `a_log` | scalar | FP32 | 1 | 32 B | 对齐到 32 B |
| `beta` | `[BT]` | FP32 | 1 | 256 B | chunk 常驻，K 循环外搬一次 |
| `in_q` | `[BT,BK]` | BF16 | 2 | 16 KiB | GQA 按 `H_k` 读 |
| `in_k` | `[BT,BK]` | BF16 | 2 | 16 KiB | 同上 |
| `out` | `[BT,BK]` | BF16 | 2 | 16 KiB | 依次写 `qg`、`kbg`、`kg` |
| `gk_last` | `[BK]` | FP32 | 1 | 256 B | 本 tile 最后一行，供 `kg` |
...
flowchart LR
  ...
  gcorr --> gk["cumsum/ln2 覆盖 work"]
  gk --> qg["qg = q * exp2(gk)"]
  gk --> kbg["kbg = k * β * exp2(gk)"]
  gk --> kg["kg = k * exp2(gk_last − gk)"]
```

1. `g` → FP32，加 `dt_bias`，safe gate `lower_bound * sigmoid(exp(A_log)*x)` → **`g_corr` 写在 `work`**
2. 沿 `BT` 前缀和 × `1/ln2` → **`gk` 覆盖 `work`**，最后一行拷到 `gk_last`
3. `exp2(gk)` 进 `tmp`；`qg = q * tmp`、`kbg = k * β * tmp` 经 `out` 写 GM
4. `kg = k * exp2(gk_last - gk)`，复用 `tmp` 和 `out`
...
| 缓冲 | 内容 | dtype | 字节 |
| --- | --- | --- | ---: |
| L1 `A` | `[64,64]` NZ | BF16 | 8 KiB |
| L1 `B0/B1` | `vb` 或 `kbg` 的 `[64,64]` ping-pong | BF16 | 16 KiB |
| L0A | `A` 的 64×64 | BF16 | 8 KiB |
| L0B | 当前 `B` tile | BF16 | 8 KiB |
| L0C | `[64,64]` 累加 | FP32 | 16 KiB |

L1 峰值约 **24 KiB**。`V=128` / `K=128` 时 N 维走两拍 64。先 `u = A @ vb`（两拍 BV），再 `w = A @ kbg`（两拍 BK）。Cube 不能依赖本 stage 的 cube 输出，所以两趟 GEMM 都读 V0 已经写好的 workspace，不读本 stage 刚写的 `u/w`。
