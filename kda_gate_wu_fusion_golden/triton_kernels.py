...
V0 is AIV-equivalent: safe-gate correct g, then chunk cumsum, then qg/kbg/vb/kg.
C0 is AIC-equivalent: u = A @ vb and w = A @ kbg with A reused.
"""
...
def v0_kernel(
    ...
    p_b = beta + (bos + o_t) * HV + i_hv
    b_b = tl.load(p_b, mask=m_t, other=0.0).to(tl.float32)

    for i_v in range(tl.cdiv(V, BV)):
        o_v = i_v * BV + tl.arange(0, BV)
        m_v = m_t[:, None] & (o_v[None, :] < V)
        p_v = v + ((bos + o_t) * HV + i_hv)[:, None] * V + o_v[None, :]
        p_vb = vb + ((bos + o_t) * HV + i_hv)[:, None] * V + o_v[None, :]
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        p_g = g + ((bos + o_t) * HV + i_hv)[:, None] * K + o_k[None, :]
        ...
        p_gk = gk + ((bos + o_t) * HV + i_hv)[:, None] * K + o_k[None, :]
        tl.store(p_gk, b_gk.to(p_gk.dtype.element_ty), mask=m_tk)

        b_e2 = exp2(b_gk)
        p_q = q + ((bos + o_t) * H + i_h)[:, None] * K + o_k[None, :]
        p_k = k + ((bos + o_t) * H + i_h)[:, None] * K + o_k[None, :]
        p_qg = qg + ((bos + o_t) * HV + i_hv)[:, None] * K + o_k[None, :]
        p_kbg = kbg + ((bos + o_t) * HV + i_hv)[:, None] * K + o_k[None, :]
        p_kg = kg + ((bos + o_t) * HV + i_hv)[:, None] * K + o_k[None, :]
        ...
        p_g_corr = g_corr + ((bos + o_t) * HV + i_hv)[:, None] * K + o_k[None, :]
...
def c0_kernel(
    ...
    for i_v in range(tl.cdiv(V, BV)):
        ...
        m_v = m_t[:, None] & (o_v[None, :] < V)
        p_vb = vb + ((bos + o_t) * HV + i_hv)[:, None] * V + o_v[None, :]
        p_u = u + ((bos + o_t) * HV + i_hv)[:, None] * V + o_v[None, :]
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        p_kbg = kbg + ((bos + o_t) * HV + i_hv)[:, None] * K + o_k[None, :]
        p_w = w + ((bos + o_t) * HV + i_hv)[:, None] * K + o_k[None, :]
...
def run_staged(
    ...
    *,
    ...
    gk = torch.empty(B, T, HV, K, device=g.device, dtype=torch.float32)
    g_corr = torch.empty(B, T, HV, K, device=g.device, dtype=torch.float32)
    qg = torch.empty(B, T, HV, K, device=q.device, dtype=q.dtype)
    kbg = torch.empty(B, T, HV, K, device=k.device, dtype=k.dtype)
    vb = torch.empty(B, T, HV, V, device=v.device, dtype=v.dtype)
