...
ge::graphStatus Tiling4ChunkKdaBwdRecompute(gert::TilingContext *context)
{
    ...
    {
        FILE *fp = fopen("/tmp/chunk_kda_bwd_recompute_tiling.log", "w");
        if (fp != nullptr) {
            fprintf(fp,
                    "B=%ld Hk=%ld Hv=%ld T=%ld K=%ld V=%ld chunkNum=%ld chunkSize=%ld "
                    "useGate=%ld useExp2=%ld hasALog=%ld hasDtBias=%ld vecRow=%ld "
                    "lbBits=%ld sizeof=%zu key=%lu ws=%zu\n",
                    tiling->B, tiling->Hk, tiling->Hv, tiling->T, tiling->K, tiling->V,
                    tiling->chunkNum, tiling->chunkSize, tiling->useGateInKernel, tiling->useExp2,
                    tiling->hasALog, tiling->hasDtBias, tiling->vecRow, tiling->lowerBoundBits,
                    sizeof(*tiling), static_cast<unsigned long>(processor.GetTilingKey()),
                    processor.GetWorkspaceSize());
            fclose(fp);
        }
    }
