...
private:
    ...
    ge::graphStatus SetVecRow()
    {
        ...
        const int64_t kbgBytes = tiling_.B * tiling_.Hv * tiling_.T * tiling_.K * static_cast<int64_t>(sizeof(uint16_t));
        const int64_t vbBytes = tiling_.B * tiling_.Hv * tiling_.T * tiling_.V * static_cast<int64_t>(sizeof(uint16_t));
        userWorkspaceSize_ = static_cast<size_t>(kbgBytes + vbBytes);
